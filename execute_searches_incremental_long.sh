    ...
    for ds in "${datasets[@]}"; do
        for seed in "${seeds[@]}"; do
            ...
            for acquisition in "${acquisitions[@]}"; do
                echo incremental_search.py --long --name "$name" --data "$ds" --experiment "$experiment" --seed "$seed" --thompson_samples "$thompson_samples" --samples_per_thompson "$samples_per_thompson" --acquisition "$acquisition" --datadir "$DATADIR" --outdir "$OUTDIR"
