...
fig, (mean_ax, std_ax) = plt.subplots(2, 1, sharex=True, figsize=(text_width * (text_width / 6.98), text_height/1.07))
...
for j, key in enumerate(sorted(results[0]['data'].keys())):
    ...
    for i, result in enumerate(sorted(results, key=lambda x: custom_key_sort(x), reverse=True)):
        ...
        mean_ax.plot(result['data'][key]['iterations'],
                     result['data'][key]['mean'], c=colors[j], linestyle=linestyles[i],
                     label=key[0] + " " + " ".join(result['name'].replace('_', ' ').split(' ')[:2]))
        mean_ax.fill_between(result['data'][key]['iterations'],
                             result['data'][key]['mean'] + result['data'][key]['mean_std'],
                             result['data'][key]['mean'] - result['data'][key]['mean_std'], color=colors[j], alpha=0.1)
        std_ax.plot(result['data'][key]['iterations'], result['data'][key]['std'],
                    c=colors[j], linestyle=linestyles[i], label=result['name'].replace('_', ' '))
        std_ax.set_xlabel('Total points samples')
...
mean_lims = {
    ...
    'protein-tertiary-structure': [-2.8, -2.74],
    'yacht': [-1, -0.4],
    'synthetic': [1.3, 1.7]
}
...
std_lims = {
    ...
    'protein-tertiary-structure': [-0.001, 0.04],
    'yacht': [0, 0.4],
    'synthetic': [0, 0.3]
}
...
savefig_handle(fig, os.path.join(outdir, f'search_algorithm_comparison_{args.experiment.split("-")[-1]}'), pdf=True, svg=False)
savefig_handle(fig, os.path.join(final_thesis_dir, args.data, f'search_algorithm_comparison_{args.experiment.split("-")[-1]}'), png=False, pdf=True, svg=False)
